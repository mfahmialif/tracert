...
onMounted(async () => {
  try {
    const response = await api.get(`/admin/years/${id}`);
    form.value = {
      code: response.data.code,
      name: response.data.name,
      smt: response.data.smt,
      is_active: Boolean(response.data.is_active),
    };
  } catch (err) {
    console.error("Failed to fetch year", err);
    error.value = "Gagal memuat data tahun akademik";
  }
});
...
<template>
  <AdminLayout>
    <div class="flex items-center gap-4 mb-6">
      ...
      <div>
        <h1 class="text-3xl font-bold tracking-tight">Edit Tahun Akademik</h1>
        <p class="text-muted-foreground">Perbarui data tahun akademik</p>
      </div>
    </div>
    ...
    <Card class="max-w-xl mx-auto">
      <CardHeader>
        ...
        <CardDescription
          >Perbarui detail tahun akademik di bawah ini.</CardDescription
        >
      </CardHeader>
